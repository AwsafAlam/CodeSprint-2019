...
def AmzonParser(url, inn):
    ...
    page = requests.get(url,headers=headers, timeout=3.0)
    while True:
        jj=random.randint(3,10)
        sleep(jj)
        try:
...
def ReadAsin():
    ...
    for i in AsinList:
        ...
        # extracted_data.append(AmzonParser(url, str(i['B00119TXES'])))
